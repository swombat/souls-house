...
**Adapt the deployment configuration before using it.** It names the upstream operator's host, SSH user/port, registry namespace, domain, and build destination. These are destinations, not credentials or permission to access them. `.kamal/secrets` retrieves secrets from local, ignored key files that a fresh clone does not include. Treat the configuration and deployment scripts as examples to adapt in this operator's fork, not universal install commands.

Make and review these changes before any Kamal command. Every destination and account must refer to the operator's installation:

- **Host and images:** replace every web/jobs/accessory host, SSH setting, builder remote, application registry namespace, cache image, and embedding image namespace in `config/deploy.yml`. Use the operator's image registry account.
- **Public identity:** set your domain in Kamal proxy settings, Rails mailer URL options, sender address, and integration callbacks. Search for `souls.house`, `dtenner`, legacy `helix_kit` names, and literal IPs; distinguish destinations that must change from internal compatibility identifiers that should remain.
- **Docker access:** preserve the required web/jobs socket mounts, but replace the hard-coded `group-add` value with the actual host Docker socket group ID. Check it with `stat -c '%g' /var/run/docker.sock` on Linux.
- **Resident networking:** set `SOULSHOUSE_AGENTS_NETWORK`, `SOULSHOUSE_AGENT_INTERNAL_URL`, `SOULSHOUSE_ACTIVITY_ORIGIN`, and `SOULSHOUSE_SANDBOX_HOST` for your deployment. The internal URL must resolve from resident containers; the activity origin must be your HTTPS origin. Keep runtime ports private.
- **Fresh production secrets:** create fresh production Rails credentials/key for this installation; adapt `.kamal/secrets` to retrieve only the operator's secrets. Supply registry credentials, `DATABASE_URL`, `POSTGRES_PASSWORD`, and the embedding bearer token through private files or a secret manager. Do not reuse upstream encrypted files/keys.
- **PostgreSQL:** `config/database.yml` expects primary, cache, queue, and cable databases; the latter three append suffixes to the primary URL's database name. Ensure all four are prepared and accessible using your credentials. Persist the PostgreSQL data directory.
- **Uploads and email:** production uses S3 storage. Configure the operator's `aws` upload bucket/region/access credentials and backup bucket, plus `smtp` relay credentials. Verify delivered confirmation/reset links point to the chosen hostname. A local-disk alternative requires deliberate storage configuration, a persistent mount, and backup coverage.
- **Embeddings:** build and publish `services/mnemodyne-embeddings` to your registry. Set `MNEMODYNE_EMBEDDING_IMAGE_DIGEST` to its actual published `sha256:…` manifest digest before **every** Kamal command; the deployment config requires it. Share a fresh private token between web/jobs and the accessory. Follow the [release checklist](https://github.com/swombat/souls-house/blob/master/docs/mnemodyne-deployment.md).
- **Resident image:** build the in-repository `agent-runtime` at its pinned Chaos commit and make the image available on the resident host. Set `SOULSHOUSE_AGENT_IMAGE_DEFAULT` accordingly. **Do not invoke `scripts/build-agent-runtime` with its defaults:** it targets the upstream operator's remote Docker host. Explicitly set this installation's target/image variables after reviewing the script, or build directly on its host with the pinned build arguments.

Once that review is complete and the operator approves deployment, use the repository's bundled Kamal and [Kamal's setup procedure](https://kamal-deploy.org/docs/commands/setup/) for the initial deployment (`bundle exec kamal setup`). Confirm the rendered destinations first without dumping secret values. Follow its output through accessory readiness, database preparation/migrations, web health, and worker startup. An app health check alone is not completion.
