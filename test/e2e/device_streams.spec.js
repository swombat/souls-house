...
test('account services leads to account-scoped device controls and separate recovery', async ({ page, request }) => {
  ...
  try {
    ...
    await page.goto(`/accounts/${setup.account_id}/services`);
    ...
    await expect(page).toHaveURL(`/accounts/${setup.account_id}/device_streams`);
    ...
    await expect(page).toHaveURL(new RegExp(`/accounts/${setup.account_id}/device_streams/[a-f0-9]+$`));
