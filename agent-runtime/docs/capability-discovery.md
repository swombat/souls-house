...
Presence on PATH is not an end-to-end access test. Consult local help before
using a helper; examples in a manual do not establish your granted scopes.
For Google Workspace, use `gws --help` (or `gws drive --help`) for local discovery,
not `soulshouse-gws --help`: the wrapper obtains an access token from the broker
before invoking `gws`, even for help. Its failure without credentials does not
mean the underlying CLI is unavailable.
